...
std::string vix::VixConfig::get_ipv4() {
  std::string address =
      this->_config_json.FindMember("bind address")->value.GetString();
...
bool vix::VixConfig::suck_in_configs(const char *path) {
  ...
  /* read the whole file */
  if_config.seekg(0, std::ios::end);
  config_buf.reserve(if_config.tellg());
  if_config.seekg(0, std::ios::beg);
  config_buf.assign(std::istreambuf_iterator<char>(if_config),
